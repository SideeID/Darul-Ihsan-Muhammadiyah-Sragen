<x-app-layout>
                            ...
                            <div class="modal-content">
                                <div class="pb-0 border-0 modal-header">
                                    <h1 class="text-center modal-title fw-700 text-dark w-100 fs-5"
                                        id="modalConfirmLabel">Konfirmasi Simpan Perubahan</h1>
                                </div>
    ...
    <script>
        ...
        var settingsGetDetail = {
            "url": `${path}/api/lowongan-kerja/detail/${id_loker}`,
            "method": "GET",
            "timeout": 0,
        };
        ...
        $.ajax(settingsGetDetail).done(function(response) {
            ...
            $('#posisi').val(data_detail.posisi);
            $('#tanggalDibuka').val(data_detail.tanggal_dibuka);
            $('#tanggalDitutup').val(data_detail.tanggal_ditutup);
            ...
            $('#foto').on('change', function(event) {
                ...
                if (file) {
                    ...
                    var reader = new FileReader();
                    reader.onload = function(e) {
                        $('#previewFoto').attr('src', e.target.result).show(); // Menampilkan gambar
                    };
                    ...
                    if (publish_sekarang === true) {
                        ...
                        $.ajax(publishSettings).done(function(response) {
                            ...
                            if (res_publish.status === "success") {
                                ...
                                "{{ route('admin.informasi.loker.index') }}";
                            } else {
                                closeLoading();
