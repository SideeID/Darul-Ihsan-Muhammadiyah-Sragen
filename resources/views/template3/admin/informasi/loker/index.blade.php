<x-app-layout>
    <div class="container-lg lowongan-kerja">
        <header class="header-view d-flex align-items-end justify-content-between">
            <div>
                <h3 class="fw-700">Lowongan Kerja</h3>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">Informasi</li>
                        <li class="breadcrumb-item">Lowongan Kerja</li>
                        <li class="breadcrumb-item active" aria-current="page">Data Lowongan Kerja</li>
                    </ol>
                </nav>
            </div>
        </header>
        ...
        <div class="mb-4 card card-main">
            <div
                ...
                <h5 class="mb-3 fw-600 text-dark mb-sm-0 h6">Data Lowongan Kerja</h5>
                <div class="d-flex">
                    <a href="{{ route('admin.informasi.loker.tambah') }}" class="btn btn-primary fw-600 btn-tambah"><img
                    ...
                    <table class="table mb-0 table-scrollable" id="table">
                        <thead>
                            <tr>
                                ...
                                <th class="align-middle fw-bold" scope="col">Posisi</th>
                                <th class="align-middle fw-bold" scope="col">Batas waktu lowongan</th>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p class="mb-4 text-center text-black">Apakah anda yakin ingin menghapus data
                            ini? </p>
    ...
    <script>
        ...
        function resetFilter() {
            ...
            setTable(
                `{{ route('lowongan-kerja.all') }}?class=table-data&table=true`
            )
        }
        ...
        function changeFilter() {
            ...
            setTable(
                `{{ route('lowongan-kerja.all') }}?class=table-data&table=true&status=${filterStatus}&tanggal=${filterTanggal}`
                )
        }

        setTableData = {
            'table-data': ['_increment', 'posisi', 'tanggal_ditutup', 'badge', 'is_action']
        }
        ...
        $(function() {
            setTable(`{{ route('lowongan-kerja.all') }}?class=table-data&table=true`)

            req({
                url: `${path}/api/lowongan-kerja/list-all`,
                type: "GET",
        ...
        $("#submitForm").click(function(e) {
            ...
            if (sorotanCount === 3 && status === "true") {
                ...
                req({
                    url: `${path}/api/lowongan-kerja/sorotan/${id}?sorotan=${status === "true" ? "true" : "false"}`,
                    type: "GET",
                    success: function(data) {
                        $('#modalConfirm').modal('hide');
                        setTable(`{{ route('lowongan-kerja.all') }}?class=table-data&table=true`)
        ...
        $('#buttonDelete').click(function(e) {
            ...
            req({
                url: `${path}/api/lowongan-kerja/delete/${id}`,
                type: "DELETE",
                success: function(data) {
                    $('#modalDelete').modal('hide');
                    setTable(`{{ route('lowongan-kerja.all') }}?class=table-data&table=true`)
