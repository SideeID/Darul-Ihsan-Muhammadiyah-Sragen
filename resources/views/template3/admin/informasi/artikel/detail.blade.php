<x-app-layout>
    <div class="container-lg artikel">
        <div class="mb-4 card card-main">
            <div class="px-4 pt-4 pb-0 bg-white border-0 card-header d-flex flex-column">
                <a href="{{ route('informasi.artikel.index') }}"
                    class="mb-3 d-flex align-items-center text-dark text-decoration-none">
                    <img src="{{ asset('image/icon/kembali.svg') }}" class="me-1" alt="icon kembali">
                    Kembali
                </a>
                <h5 class="mb-3 fw-700 text-dark mb-sm-0">Detail Artikel</h5>
            </div>
            <div class="p-4 pt-0 overflow-auto card-body">
                <hr class="my-4">
                <form class="form-artikel" id='form_artikel' action="" enctype="multipart/form-data"
                    method="post">
                    @csrf

                    <p class="mb-4 text-black fw-700">Gambar Artikel</p>
                    <div class="mb-4 img-preview mb-md-5">
                        <input type="file" hidden name="foto" id="foto" accept="image/png, image/jpg"
                        ...
                        <label for="foto"
                            ...
                            <div class="label-content">
                                <p class="mb-2 text-center text-black fw-700">Drag & drop image to upload, or <span
                                        class="text-primary">browse</span></p>
                                <small class="text-center text-secondary d-block">Gunakan dimensi <b>800x600px</b>
                                    dengan ukuran maksimal <b>2MB</b>
                                    dengan ekstensi file <b>PNG</b> atau <b>JPG</b>.</small>
                            </div>
                        </label>
                    </div>

                    <p class="mb-4 text-black fw-700">Detail Artikel</p>
                    <div class="mb-3 form-group position-relative mb-md-3">
                        <label for="judul" class="form-label">Judul Artikel</label>
                        <input type="text" id="judul" name="judul" class="form-control"
                            placeholder="Judul Artikel" aria-label="Judul Artikel">
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                    ...
                    <div class="mb-3 form-group position-relative mb-md-3">
                        <textarea name="isi" id="isi" class="form-control" placeholder="Isi Artikel" aria-label="Isi Artikel"
                            style="min-height: 235.336px"></textarea>
                    </div>
                    <div class="mb-3 form-group position-relative mb-md-3" id="checkboxPublish">
                    ...
                    <button class="btn btn-success w-100 fw-500" type="button" data-bs-toggle="modal"
                        data-bs-target="#modalConfirm" id="modalConfirmTrigger">SIMPAN DATA ARTIKEL</button>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="p-4 modal-body">
                                    <p class="mb-4 text-black">Apakah data yang anda masukkan sudah benar?, Anda bisa
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <button type="button" class="p-2 btn btn-outline-success fw-700 w-100"
                                            id="submitPublish">Simpan Data</button>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        var id_artikel = {!! json_encode($data) !!};
        let foto_value
        var isi;
            ...
            if (selectedFile) {
                if (selectedFile.size <= 2 * 1024 * 1024) {
                    ...
                    $(".img-preview label").html(`
                        ...
                        <div class="label-content">
                            <p class="mb-2 text-center text-black fw-700">Drag & drop image to upload, or <span class="text-primary">browse</span></p>
                            <small class="text-center text-secondary d-block">Gunakan dimensi <b>800x600px</b> dengan ukuran maksimal <b>2MB</b>
                                dengan ekstensi file <b>PNG</b> atau <b>JPG</b>.</small>
                        </div>
                    `)
                }
        ...
        var settingsGetDetail = {
            "url": `${path}/api/artikel/detail/${id_artikel}`,
            "method": "GET",
            "timeout": 0,
        };
        ...
        $("#submitPublish").on("click", function(event) {
            ...
            var settings = {
                ...
                "url": `${path}/api/artikel/update/${id_artikel}`,
                "method": "POST",
                "timeout": 0,
                "processData": false,
                ...
                if (res.status === "success") {
                    ...
                    if (publish_sekarang === true) {
                        ...
                        var settings = {
                            ...
                            "url": `${path}/api/artikel/publish-one/${id}`,
                            "method": "GET",
                            "timeout": 0,
                            "processData": false,
                            "mimeType": "multipart/form-data",
                        ...
                        $.ajax(settings).done(function(response) {
                            ...
                            if (res_publish.status === "success") {
                                closeLoading();
                                window.location.href = "{{ route('informasi.artikel.index') }}";
                            } else {
                                closeLoading();
