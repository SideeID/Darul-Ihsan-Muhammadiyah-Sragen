...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('template3/css/tentangkami.css') }}">
    ...
    <style>
        ...
        .sidebar-footer .dropdown-menu .dropdown-item img {
            margin-right: 10px;
            filter: invert(0);
            /* Menghilangkan warna icon */
            transition: filter 0.3s;
        }

        .sidebar-footer .dropdown-menu .dropdown-item:hover img {
            filter: invert(1);
            /* Membuat icon menjadi putih saat hover */
        }
        ...
        .sidebar {
            transition: all 0.3s ease;
            width: 250px;
            /* Sesuaikan dengan lebar asli sidebar */
        }

        .sidebar.hide {
            width: 80px;
            /* Lebar sidebar saat dikecilkan */
        }
        ...
        .shrink .sidebar-brand-large {
            display: none;
        }

        .shrink .sidebar-brand-small {
            display: block;
        }
        ...
        .main-content.sidebar-hidden {
            margin-left: 60px;
            /* Sesuaikan dengan lebar sidebar saat dikecilkan */
        }
    </style>
    ...
    <script>
        ...
        var arrBulan = ['', 'JANUARI', 'FEBRUARI', 'MARET', 'APRIL', 'MEI', 'JUNI', 'JULI', 'AGUSTUS', 'SEPTEMBER',
        ...
        function req(opt = null) {
            $.ajax({
                ...
                if (urlChecking[2] == 'monitoring' && (urlChecking[3] == 'pendapatan' || urlChecking[3] ==
                        'potongan')) {
                    if (opt.url.pathname == '/api/review/gaji-pegawai' || opt.url.pathname ==
...
<body>
    ...
    <div class="wrapper d-flex flex-column min-vh-100 bg-light">
        <div class="alert fw-500 alert-dismissible d-none alert-success" role="alert" id="set_alert"
            style="z-index: 1056;">
            ...
            <div class="container-lg">
                <section class="d-flex align-items-sm-center justify-content-between body-header">
                    <button class="p-0 header-toggler" type="button" id="btn_sidebar">
                        <img src="{{ asset('image/icon/menu-hamburger.svg') }}" class="icon icon-lg"
                            alt="icon toggle sidebar">
                    </button>
                    <button class="p-0 bg-transparent border-0 btn position-relative toggle-notifikasi"
                        id="toggle_notifikasi" type="button" data-bs-toggle="offcanvas"
                        data-bs-target="#notifikasi" aria-controls="notifikasi">
    ...
    <!-- Notfikasi -->
    <div class="offcanvas offcanvas-end notifikasi" tabindex="-1" id="notifikasi"
        aria-labelledby="notifikasiLabel">
        ...
        <button class="p-2 mb-4 btn btn-outline-success btn-read w-100 fw-700" onclick="readNotif('all')"
            id="read_all">Tandai semua sudah dilihat</button>
            ...
            <div class="modal-content">
                ...
                <div class="p-4 modal-body">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <button type="button" class="p-2 btn btn-secondary fw-700 me-3 w-50"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="p-4 modal-body">
                    ...
                    <div class="d-flex align-items-center">
                        <button type="button" class="p-2 btn btn-danger fw-700 me-3 w-100"
                            data-bs-dismiss="modal">Batal</button>
                        <button type="button" class="p-2 btn btn-outline-success fw-700 w-100"
                            id="modalConfirmOk">Ya</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        async function setTable(url, callback = null, filtered = null) {
            ...
            var loading =
                `<tr><td colspan="${setTableData[class_table].length}" class="text-center text-muted" style="font-size: 1rem; background-color: #eff3f8;">.: Mengambil Data :.</td</tr>`;
            ...
            req({
                ...
                success: function(response) {
                    ...
                    for (var i = 1; i <= data_table.last_page; i++) {
                        _option +=
                            `<option value="${i}"${(i === data_table.current_page ? ' selected' : '')}>${i}</option>`;
                    }
                    page_select
                        .html(_option)
                        .attr('onchange',
                            `setTablePage("${class_table}", "${data_table.first_page_url}", ${null}, ${callback})`
                        );

                    // button prev and next
                    $(`.${class_table} .btn-prev`).attr('onclick',
                        `setTable("${ (data_table.prev_page_url || '#') }", ${callback}, ${filtered})`);
                    $(`.${class_table} .btn-next`).attr('onclick',
                        `setTable("${ (data_table.next_page_url || '#') }", ${callback}, ${filtered})`);

                    // length data select
                    show_length.attr('onchange',
                        `setTable("${data_table.first_page_url}", ${callback}, ${filtered})`);

                    // input search
                    search_input.attr('onchange',
                        `setTablePage("${class_table}", "${data_table.first_page_url}", 1, ${callback})`
                    );
                    $(`.${class_table} .table-btn-search`).attr('onclick',
                        `setTable("${url}", ${callback}, ${filtered})`);
        ...
        function tanggal(tgl) {
            ...
            const arr_month = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September',
                'Oktober', 'November', 'Desember'
            ];
