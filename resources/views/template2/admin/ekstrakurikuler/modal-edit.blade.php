    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="media-upload" id="edit-upload-section">
                    <label for="edit-upload" class="media-link">Thumbnail</label>
                    <div class="upload-box" onclick="document.getElementById('edit-upload').click();">
                        <img src="{{asset('template2/assets/image/dashboard-ekskul/gallery-add.svg')}}" alt="Upload Icon" id="edit-upload-icon">
                        <input type="file" id="edit-upload" accept="image/png, image/jpeg" style="display: none;" onchange="showEditSelectedFile(event)">
                        <p>Upload image, or <a href="#" onclick="document.getElementById('edit-upload').click();">Browse</a></p>
                        <span>1920x1080px size required in PNG or<br>JPG format only.</span>
                    </div>
                </div>
                {{-- preview --}}
                <div id="edit-completed-section" style="display: none; position: relative;">
                    <label for="edit-thumbnail-preview" class="media-link">Thumbnail Preview</label>
                    <img id="edit-thumbnail-preview" src="" alt="Selected Thumbnail" style="width: 100%; height: auto; border-radius: 8px; border: 2px dashed #ccc; margin-bottom: 10px;">
                    {{-- btn --}}
                    <div style="position: absolute; top: 35px; right: 15px; display: flex; gap: 10px;">
                        ...
                        <button class="btn btn-outline-danger btn-sm" onclick="deleteImage()"><img src="{{ asset('image/icon/icon-delete.svg') }}" alt="delete-gambar"></button>
                    </div>
                </div>

                <div class="mt-3 form-group">
                    <label for="edit-nama-ekskul" class="nama-ekskul">Nama Ekstrakurikuler</label>
                    <input type="text" class="form-control" id="edit-nama-ekskul" placeholder="e.g; Pramuka" onblur="validateEditInput()">
                    <small id="edit-error-msg" style="color: red; display: none;">Kolom nama ekstrakurikuler wajib diisi!</small>
                </div>
                <div class="mt-1 form-group">
                    <label for="edit-url" class="url">Link URL</label>
                    <input type="text" class="form-control" id="edit-url" placeholder="e.g; https://" onblur="validateEditInput()">
                    <small id="edit-error-msg-url" style="color: red; display: none;">Kolom link url wajib diisi!</small>
                </div>
                <div class="mt-3 form-check">
                    <input class="form-check-input" type="checkbox" id="edit-publish-checkbox">
                    <label class="form-check-label" for="edit-publish-checkbox">
...
<script>
    ...
    function validateEditInput() {
        const namaEkskul = document.getElementById('edit-nama-ekskul').value.trim();
        const url = document.getElementById('edit-url').value.trim();

        hideInputError('edit-nama-ekskul', 'edit-error-msg');
        hideInputError('edit-url', 'edit-error-msg-url');

        let isValid = true;

        if (namaEkskul === '') {
            showInputError('edit-nama-ekskul', 'edit-error-msg', 'Kolom nama ekstrakurikuler wajib diisi!');
            isValid = false;
        }

        if (url === '') {
            showInputError('edit-url', 'edit-error-msg-url', 'Kolom Link URL wajib diisi!');
            isValid = false;
        }
    ...
    function openEditModal(id) {
        ...
        fetch(`${path}/api/ekskul/detail/${id}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById('edit-nama-ekskul').value = data.data.nama;
                document.getElementById('edit-url').value = data.data.url;
                document.getElementById('edit-publish-checkbox').checked = data.data.status === 'published';

                if (data.data.thumbnail) {
                    document.getElementById('edit-upload-section').style.display = 'none';
                    document.getElementById('edit-completed-section').style.display = 'block';
                    document.getElementById('edit-thumbnail-preview').src = data.data.thumbnail;
                }
    ...
    function updateData() {
        ...
        formData.append('status', document.getElementById('edit-publish-checkbox').checked ? 'published' : 'waiting');
        ...
        fetch(`${path}/api/ekskul/update/${id}`, {
                method: 'POST',
                body: formData,
                headers: {
            ...
            .then(data => {
                ...
                setTable(`${path}/api/ekskul/list-all?class=table-data&table=true`);
    ...
    function resetEditForm() {
        document.getElementById('edit-nama-ekskul').value = '';
        document.getElementById('edit-url').value = '';
        document.getElementById('edit-publish-checkbox').checked = false;
        document.getElementById('edit-upload').value = '';
        document.getElementById('edit-upload-section').style.display = 'block';
        document.getElementById('edit-completed-section').style.display = 'none';
        document.getElementById('edit-thumbnail-preview').src = '';
    ...
    document.getElementById('edit-nama-ekskul').addEventListener('input', function() {
...
@include(config('app.template') . 'admin.ekstrakurikuler.modal-notifedit')
