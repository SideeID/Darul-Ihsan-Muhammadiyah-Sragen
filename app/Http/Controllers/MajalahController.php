...
class MajalahController extends Controller
{
    public function list(Request $req)
    {
        try {
            ...
            $query = Majalah::orderBy('created_at', 'desc');
            ...
            foreach ($data as $d => $v) {
                ...
                "<a href='" . route('informasi.majalah.detail', $v->id) . "' class='btn btn-outline-black me-2' data-bs-toggle='tooltip' data-bs-title='Detail'><img src='" . asset('image/icon/icon-detail-black.svg') . "' alt='detail'></a>" .
    ...
    public function store(Request $req)
    {
        try {
            ...
            if ($req->hasFile('image')) {
                ...
                $path = public_path() . '/storage/path/majalah';
                $req->image->move($path, $namafile);
                $imagePath = '/storage/path/majalah/' . $namafile;
            }
    ...
    public function update(Request $req, $id)
    {
        try {
            ...
            if ($req->hasFile('image')) {
                if (file_exists(public_path($Majalah->image))) {
                    unlink(public_path($Majalah->image));
                }
            ...
            $Majalah->update([
                ...
                'penulis' => $req->penulis,
                'url' => $req->url,
                'image' => $Majalah->image ?? $Majalah->getOriginal('image'),
                'tanggal' => $req->tanggal,
                'status' => $req->status,
            ]);

            return apiSuccess($Majalah, 'Data berhasil diubah');
        } catch (\Exception $e) {
            return apiFailed($e->getMessage(), 500);
        }
    }
    ...
    public function publish(Request $req)
    {
        try {
            ...
            $data = Majalah::whereIn('id', $req->ids)->update(['status' => 'published']);
